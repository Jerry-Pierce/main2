...
def main_page():
    ...
    # 세션 디버깅 로그 추가
    print(f"🔍 메인 페이지 접근: logged_in={session.get('logged_in', False)}, user_id={session.get('user_id')}, username={session.get('username')}")
    print(f"🔍 세션 전체 내용: {dict(session)}")
    print(f"🔍 세션 영구 설정: {session.permanent if hasattr(session, 'permanent') else 'N/A'}")
    ...
    <body>
        <div class="container">
            ...
            <div style="color: #888; font-size: 0.9rem; margin-bottom: 20px; font-style: italic;">
                ''' + ('빠르고 간편한 URL 단축 서비스' if not is_logged_in() else f'환영합니다, {session.get("username", "사용자")}님! 🎉') + '''
            </div>
